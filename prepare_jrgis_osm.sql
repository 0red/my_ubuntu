...
DECLARE 
 ...
 BEGIN
	...
	RAISE NOTICE 'jrgis.createtables()';
...
create index pl_railway_poly_idx on jrgis.pl_railway_poly(osm_id);	RAISE NOTICE 'DROP TIME %',clock_timestamp() - t1;
...
create index pl_admin_poly_ldx on jrgis.pl_admin_poly(osm_id,admin_level);	RAISE NOTICE 'DROP TIME %',clock_timestamp() - t1;
...
create index pl_railway_rdx on jrgis.pl_railway(osm_id,ref);	RAISE NOTICE 'DROP TIME %',clock_timestamp() - t1;
...
create index pl_water_poly_wdx on jrgis.pl_water_poly(osm_id,water);	RAISE NOTICE 'DROP TIME %',clock_timestamp() - t1;
